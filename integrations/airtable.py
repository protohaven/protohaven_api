...
def insert_records(data, base, tbl):
    ...
    post_data = dict(records=[dict(fields=d) for d in data])
...
def update_record(data, base, tbl, rec):
    ...
    response = requests.request(
        "PATCH", url, headers=headers, data=json.dumps(post_data)
    )
    return response
...
def get_clearance_to_tool_map():
    ...
    tool_code_by_id = dict(
        [(t["id"], t["fields"]["Tool Code"]) for t in airtable_tools]
    )
