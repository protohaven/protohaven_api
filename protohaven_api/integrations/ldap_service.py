...
class NeonLDAPEntry(LDIFTreeEntry):
    ...
    def __init__(
        self, dn: str, attributes: Dict[str, List[bytes]], service: "NeonLDAPService"
    ):
        self.dn = dn
        ...
        for key, values in attributes.items():
            if isinstance(values, list):
                ...
                self._attributes[key] = [
                    values.encode("utf-8") if isinstance(values, str) else values
                ]
...
class NeonLDAPTree:
    ...
    def lookup(self, dn: str) -> Optional[NeonLDAPEntry]:
        ...
        # Handle user entries
        if dn_lower.startswith("uid=") and dn_lower.endswith(
            f",{self.users_dn.lower()}"
        ):
    ...
    def search(
        self, base_dn: str, scope: int, filter_obj, attributes: List[str] = None
    ) -> List[NeonLDAPEntry]:
        ...
        # Subtree search
            ...
            if (
                base_dn_lower == self.base_dn.lower()
                or base_dn_lower == self.users_dn.lower()
            ):
        ...
        log.info(
            f"LDAP search base={base_dn} scope={scope} returned {len(results)} results"
        )
        return results
...
class NeonLDAPServer(LDAPServer):
    ...
    def handle_LDAPSearchRequest(self, request, controls, reply):
        ...
        try:
            ...
            attributes = (
                [str(attr) for attr in request.attributes]
            ...
            for entry in results:
                ...
                if attributes and "*" not in attributes:
                    entry_attributes = {
                        k: v for k, v in entry_attributes.items() if k in attributes
                    }
    ...
    def _run_server(self):
        ...
        try:
            ...
            endpoint = endpoints.TCP4ServerEndpoint(
                reactor, self.port, interface=self.host
            )
...
def create_ldap_service(host, port) -> NeonLDAPService:
