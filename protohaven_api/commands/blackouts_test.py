...
def test_sync_booked_blackouts_dry_run(mocker, cli):
    ...
    # Mock dependencies
    mocker.patch.object(
        ts.forecast, "generate", return_value={"calendar_view": mock_calendar_view}
    )
    mocker.patch.object(
        ts.booked,
        "get_resources",
    ...
    mocker.patch(
        "protohaven_api.commands.blackouts.tznow",
        return_value=datetime.datetime(2024, 1, 1, 0, 0, 0),
    )
...
def test_sync_booked_blackouts_with_apply(mocker, cli):
    ...
    mocker.patch.object(
        ts.booked,
        "get_resources",
        return_value=[{"resourceId": "tool1", "name": "Test Tool"}],
    )
    mocker.patch.object(ts.booked, "get_blackouts", return_value={"blackouts": []})
    mock_create = mocker.patch.object(
        ts.booked, "create_blackout", return_value={"blackoutId": "test123"}
    )
    mocker.patch(
        "protohaven_api.commands.blackouts.tznow",
        return_value=datetime.datetime(2024, 1, 1, 0, 0, 0),
...
def test_sync_booked_blackouts_existing_blackout(mocker, cli):
    ...
    mocker.patch.object(
        ts.booked, "get_resources", return_value=[{"resourceId": "tool1"}]
    )
    mocker.patch.object(
        ts.booked,
        "get_blackouts",
    ...
    # and existing one should NOT be removed (still needed)
    assert len(result) == 1
    summary = result[0]
    assert (
        summary["blackouts_to_create"] == 0
    )  # No new blackouts needed (already exists)
    assert (
        summary["blackouts_to_remove"] == 0
    )  # Existing blackout should NOT be removed (still needed - AM has 0 techs)
...
def test_sync_booked_blackouts_remove_blackouts(mocker, cli):
    ...
    assert (
        summary["blackouts_to_remove"] == 2
    )  # Both AM and PM blackouts should be removed
