...
@pytest.fixture
def client():
    class StubClient(PHClient):
        def __init__(self):
            pass # Don't do usual init stuff
    return StubClient()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        Tc("Not excluded but not included prefers strict", "usr", True, {"usr2"}, {"usr2"}, False),
        Tc("Fail on both filters", "usr", True, {"usr2"}, {"usr"}, False),
    ], ids=idfn,
)
def test_hook_on_user_is_permitted(tc, client, mocker):
    """Test various cases of inclusion/exclusion"""
    mocker.patch.object(type(client), 'cfg', new_callable=mocker.PropertyMock, return_value={
        'event_hooks': {
            'enabled': tc.enabled,
    ...
    assert client._hook_on_user_is_permitted(tc.usr) == tc.want
