			...
			<Card class="m-2">
				...
				<CardBody>
					...
					{#if a.Survey == 'Text'}
						<Input
							type="text"
							placeholder="(Optional) please type a response"
							bind:value={a.response}
							disabled={a.submitting || a.submit_ok}
						></Input>
					{/if}
				</CardBody>
				{#if a.Survey}
					<CardFooter>
						<Button
							on:click={() => submitAnnouncementResponse(a)}
							color="primary"
							disabled={a.submitting || a.submit_ok || !a.response}>Submit</Button
						>
						{#await a.promise}
							<Spinner />
						{:then arep}
	...
	{#if !guest && reservations.length > 0}
		<Card class="m-2">
			<CardHeader>
				...
				<CardSubtitle
					>Go to <strong>reserve.protohaven.org</strong>
