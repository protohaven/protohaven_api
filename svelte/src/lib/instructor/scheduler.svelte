<script type="typescript">
	...
	function do_validate() {
		running = true;
		post(
			'/instructor/validate?email=' + encodeURIComponent(email),
			post_data(),
			...
			.catch((err) => {
				if (err.toString().startsWith('AbortError')) {
					...
					validation_result = {
						valid: false,
						errors: [`Validation request error: ${err}`, `Contact #software channel in Discord`]
					};
				}
			})
	...
	function save_schedule() {
		running = true;
		post('/instructor/push_class?email=' + encodeURIComponent(email), post_data())
			.then((rep) => {
				console.log(rep);
				if (rep.valid) {
					open = false;
			...
			.catch((err) => {
				validation_result = {
					valid: false,
					errors: [`Save error: ${err}`, `Contact #software channel in Discord`]
				};
			})
			.finally(() => (running = false));
	}
...
<Modal size="lg" isOpen={open}>
	...
	<ModalBody>
					...
					<Row>
						...
						<Col>
							<Input
								type="date"
								placeholder="Start"
								bind:value={selected.starts[i][0]}
								on:change={do_validate_debounced}
							/>
						</Col>
						<Col>
		...
		{#if validation_result.errors.length > 0}
			...
			<p>
				<em
					>Type "I own the consequences of my actions" into the box below to ignore all errors and
					proceed.</em
				>
			</p>
			...
			{#if validation_override}
				<p>
					<em
						>An automated message will be sent to the Education Leads, but <strong
							>you are responsible for telling them about this class being scheduled without
							validation.</strong
						></em
					>
				</p>
	...
	<ModalFooter>
		...
		{#if !running && validation_result.valid}All validation checks passed <Icon
				name="check-all"
			/>{/if}
		{#if selected && !running && !validation_result.valid}Some validation checks failed <Icon
				name="exclamation-triangle"
			/>
			{#if ovr}<strong> proceed anyways?</strong>{/if}
		{/if}
		<Button on:click={save_schedule} disabled={!ovr && (running || !validation_result.valid)}
			>Save</Button
		>
