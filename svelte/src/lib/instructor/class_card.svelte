	...
	<Card class="my-3" size={'lg'}>
		<CardHeader style={c.event_id ? 'background-color: rgb(230, 225, 249)' : ''}>
			<CardTitle id={schedule_id}>
				...
				{:else}
					<a href="https://protohaven.org/wiki/instructors#scheduling" target="_blank"
		...
		<CardBody>
			...
			{#if c.rejected}
				<Alert color="warning" class="mx-3"
					>This card will disappear upon refreshing the page.</Alert
				>
			{/if}
				...
				<ul>
					{#each c.sessions as ss}
						<li>
							...
							{new Date(ss[1]).toLocaleString('en-US', { timeZone: 'America/New_York' })}
						</li>
					{/each}
				</ul>
			</CardText>
				...
				<ul>
					...
					<li>
						...
						{#if c.supply_state == 'Supplies Requested'}
							<strong
								>- remember to file <a
									href="https://form.asana.com/?k=syF2O04JfU-Z82q6NcEJKg&d=1199692158232291"
									target="_blank">purchase requests</a
								></strong
							>
						{/if}
					</li>
					<li>
						Instruction: {#if c.volunteer}Volunteer (no pay){:else}Paid{/if}
					</li>
					{#if c.confirmed}
						<li>
							Proposed {new Date(c.confirmed).toLocaleString('en-US', {
		...
		<CardFooter class="d-flex">
			<Dropdown autoClose={true}>
				...
				<DropdownMenu>
					...
					<DropdownItem
						href="https://form.asana.com/?k=syF2O04JfU-Z82q6NcEJKg&d=1199692158232291"
						target="_blank">Purchase Request</DropdownItem
					>
					{#if c.volunteer}
					...
					{#if !c.event_id}
						<DropdownItem divider />
						<DropdownItem on:click={() => confirm(false)}
							>Mark unavailable (hides permanently)</DropdownItem
						>
					{:else}
						<DropdownItem divider />
