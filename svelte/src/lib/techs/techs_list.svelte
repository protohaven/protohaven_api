<script type="typescript" lang="ts">
	...
	let new_tech: { neon_id: string | null; name: string; email: string } = {
		neon_id: null,
		name: '',
		email: ''
	};
			...
			.then((data: TechListData) => {
				...
				techs = data.techs.map((t: Tech) => {
					const displayTech: DisplayTech = {
						...t,
						shop_tech_shift: Array.isArray(t.shop_tech_shift)
		...
		<CardBody>
			...
			{:then p}
				...
				<div class="d-flex">
					...
					{/if}
					<Dropdown>
						<DropdownToggle color="light" caret aria-label="Sort options">Sort</DropdownToggle>
						<DropdownMenu>
							<DropdownItem on:click={() => (sort_type = 'clearances_asc')}
								>Least Clearances</DropdownItem
							>
							<DropdownItem on:click={() => (sort_type = 'clearances_desc')}
								>Most Clearances</DropdownItem
							>
