...
docker run --rm --volumes-from nocodb_frontend -v $(pwd):/backup ubuntu tar cvf /backup/nc_data.tar /usr/app/data
echo "Snaphsotting Nocodb postgres container to dump.sql..."
docker exec -it nocodb_db pg_dump -U postgres -d root_db > dump.sql
